...
void drawSatellitePositionLabels(GLuint uiProgram, GLuint uiVao, GLuint uiVbo, const Mat4& satelliteVp, const Player& playerOne, const Player& playerTwo, int width, int height, float feedScale = 1.0f) {
    ...
    auto addLabel = [&vertices, &satelliteVp, panelX, panelY, panelW, panelH](const Player& player, int digit, std::array<float, 4> color) {
        ...
        const float bw = panelW * 0.040f;
        const float bh = panelH * 0.040f;
        ...
        addUiRect(vertices, pos.x + bw * 0.45f, pos.y + bh, bw * 0.55f, t, color);
        addUiRect(vertices, pos.x + bw, pos.y + bh * 0.45f, t, bh * 0.55f, color);
        addUiRect(vertices, pos.x - 0.011f, pos.y - 0.009f, 0.040f, 0.054f, {0.0f, 0.0f, 0.0f, 0.42f});
        addHudDigit(vertices, pos.x, pos.y, size, digit, color);
        addUiRect(vertices, pos.x - 0.008f, pos.y + 0.048f, 0.037f, 0.003f, {color[0], color[1], color[2], 0.72f});
    };
...
vec3 bumpedNormal(vec3 normal, vec3 pos, float strength) {
    vec3 helper = abs(normal.y) < 0.82 ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);
    vec3 tangent = normalize(cross(helper, normal));
    vec3 bitangent = normalize(cross(normal, tangent));
    float h1 = fbm(pos * 3.4 + vec3(7.1, 2.0, 11.3)) - 0.5;
    float h2 = fbm(pos * 3.4 + vec3(13.4, 5.7, 1.9)) - 0.5;
    return normalize(normal + (tangent * h1 + bitangent * h2) * strength);
}
...
void main() {
    ...
    if (kind == 1.0) {
        ...
        float inner = smoothstep(0.38, 0.92, fbm(p * 11.0 + uTime * 0.08));
        base = mix(vec3(0.045, 0.052, 0.060), vec3(0.34, 0.39, 0.42), shard);
        base += vec3(0.08, 0.14, 0.16) * inner;
        alpha = 0.72;
    } else if (kind == 8.0) {
        ...
        float glowPulse = 0.74 + 0.26 * sin(uTime * 3.6 + p.x * 0.7 + p.z * 0.4);
        float pulse = smoothstep(0.34, 0.92, fbm(p * 5.0 + glowPulse));
        float vein = cracks(p * 0.82);
        base = mix(vec3(0.00, 0.18, 0.58), vec3(0.00, 0.95, 1.0), pulse);
        base += vec3(0.18, 0.90, 1.0) * vein * (0.95 + glowPulse * 0.40);
    } else if (kind == 11.0) {
        float glowPulse = 0.78 + 0.22 * sin(uTime * 4.1 + p.y * 0.9 + p.z * 0.3);
        float grit = smoothstep(0.42, 0.88, fbm(p * 6.0 + glowPulse));
        float vein = cracks(p * 0.95);
        base = mix(vec3(0.36, 0.48, 0.00), vec3(1.0, 1.0, 0.00), grit);
        base += vec3(0.95, 1.0, 0.06) * vein * (0.88 + glowPulse * 0.34);
    } else if (kind == 12.0) {
        ...
        float reflection = smoothstep(0.35, 0.86, fbm(p * 3.0 + vec3(uTime * 0.08, 0.0, -uTime * 0.05)));
        base = mix(vec3(0.030, 0.050, 0.060), vec3(0.36, 0.45, 0.48), reflection);
        alpha = 0.62;
    }

    float roughBlock = (kind == 1.0 || kind == 2.0 || kind == 3.0 || kind == 4.0 || kind == 8.0 || kind == 12.0 || kind == 13.0 || kind == 14.0 || kind == 16.0 || kind == 19.0 || kind == 20.0 || kind == 21.0) ? 1.0 : 0.0;
    float polishedBlock = (kind == 7.0 || kind == 9.0 || kind == 10.0 || kind == 11.0 || kind == 22.0) ? 1.0 : 0.0;
    float livingBlock = (kind == 15.0 || kind == 17.0 || kind == 18.0) ? 1.0 : 0.0;
    ...
    if (kind == 10.0) color = mix(color, vec3(0.00, 1.0, 1.0), 0.50) + vec3(0.0, 0.35, 0.55) * fresnel;
    if (kind == 11.0) color = mix(color, vec3(1.0, 1.0, 0.02), 0.46) + vec3(0.45, 0.48, 0.00) * fresnel;
    color = mix(color, pow(color * 1.55, vec3(0.88)), uFeedClarity);
    color = colorGrade(color * 1.88 + vec3(0.018));
...
void main() {
    ...
    if (uTheme == 3) {
        ...
        float beam = pow(max(0.0, 0.34 - sunDist), 2.0);
        color += vec3(0.60, 0.48, 0.25) * beam * 2.9;
        color += vec3(0.06, 0.08, 0.07) * smoothstep(0.55, 0.88, fbm(uv * 22.0 + slow));
        color += vec3(0.12, 0.12, 0.10) * smoothstep(0.70, 0.95, smog) * low;
        FragColor = vec4(toneSky(color * 1.52, uv), 1.0);
        return;
    }
    if (uTheme == 2) {
        ...
        color += vec3(0.88, 0.72, 0.36) * glow * 3.2;
        color += vec3(0.10, 0.17, 0.20) * smoothstep(0.70, 0.94, fbm(uv * 24.0 + slow * 2.0)) * smoothstep(0.30, 1.0, uv.y);
        FragColor = vec4(toneSky(color * 1.34, uv), 1.0);
        return;
    }
    if (uTheme == 1) {
        ...
        color += vec3(1.0, 0.67, 0.24) * glare * 3.6;
        color = mix(color, vec3(1.0, 0.78, 0.34), disk * 0.72);
        color += vec3(0.24, 0.13, 0.06) * smoothstep(0.48, 0.90, highDust) * horizonBand;
        color += vec3(0.18, 0.08, 0.03) * smoothstep(0.62, 0.92, fbm(uv * 30.0 + vec2(-slow * 4.0, slow)));
        FragColor = vec4(toneSky(color * 1.42, uv), 1.0);
        return;
    }
    ...
    color += vec3(1.0, 0.22, 0.045) * sun * 6.4 * (1.0 - occlusion * 0.68);
    ...
    float starField = hash(floor(uv * vec2(520.0, 300.0)));
    float stars = step(0.996, starField) * smoothstep(0.35, 0.96, uv.y) * (1.0 - horizonBand);
    color += vec3(0.72, 0.78, 0.88) * stars * (0.22 + 0.78 * hash(floor(uv * vec2(520.0, 300.0)) + vec2(13.7)));
    ...
    FragColor = vec4(toneSky(color * 1.58, uv), 1.0);
}
)GLSL";
...
void main() {
    ...
    float shape = smoothstep(0.08, 0.88, grey);
    float contour = 1.0 - smoothstep(0.010, 0.035, abs(fract(grey * 5.0) - 0.5));
    vec3 ink = vec3(0.006, 0.010, 0.018);
    vec3 low = vec3(0.020, 0.055, 0.080);
    vec3 high = vec3(0.54, 0.82, 0.88);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (satelliteFeedDirty || missileCameraActive || gameTime - lastSatelliteFeedTime >= CameraFeedRefreshInterval) {
            ...
            if (missileCameraActive) {
                ...
                drawVoxelScene(voxelUniform, satelliteView.vp, opaque, transparentMesh, nullptr, {}, nullptr, {}, {}, {}, satelliteView.position, satelliteView.down, renderTime, 0.20f, true);
        ...
        if (satelliteFeedDirty || gameTime - lastSatelliteFeedTimeTwo >= CameraFeedRefreshInterval) {
            ...
            drawVoxelScene(voxelUniform, satelliteViewTwo.vp, opaque, transparentMesh, nullptr, {}, nullptr, {}, {}, {}, satelliteViewTwo.position, satelliteViewTwo.down, renderTime, 0.20f, true);
